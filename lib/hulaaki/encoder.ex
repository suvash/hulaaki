defmodule Hulaaki.Encoder do
  ...
  def encode_fixed_header(%Message.ConnAck{}) do
    encode_fixed_header_first_byte(2, 0, 0, 0) <> encode_fixed_header_second_byte(2)
  end

  def encode_fixed_header(%Message.Publish{dup: dup, qos: qos, retain: retain} = message) do
    remaining_length = calculate_remaining_length(message)
  ...
  def encode_fixed_header(%Message.PubAck{}) do
    encode_fixed_header_first_byte(4, 0, 0, 0) <> encode_fixed_header_second_byte(2)
  end

  def encode_fixed_header(%Message.PubRec{}) do
    encode_fixed_header_first_byte(5, 0, 0, 0) <> encode_fixed_header_second_byte(2)
  end

  def encode_fixed_header(%Message.PubRel{}) do
    encode_fixed_header_first_byte(6, 0, 1, 0) <> encode_fixed_header_second_byte(2)
  end

  def encode_fixed_header(%Message.PubComp{}) do
    encode_fixed_header_first_byte(7, 0, 0, 0) <> encode_fixed_header_second_byte(2)
  end
  ...
  def encode_fixed_header(%Message.UnsubAck{}) do
    encode_fixed_header_first_byte(11, 0, 0, 0) <> encode_fixed_header_second_byte(2)
  end

  def encode_fixed_header(%Message.PingReq{}) do
    encode_fixed_header_first_byte(12, 0, 0, 0) <> encode_fixed_header_second_byte(0)
  end

  def encode_fixed_header(%Message.PingResp{}) do
    encode_fixed_header_first_byte(13, 0, 0, 0) <> encode_fixed_header_second_byte(0)
  end

  def encode_fixed_header(%Message.Disconnect{}) do
    encode_fixed_header_first_byte(14, 0, 0, 0) <> encode_fixed_header_second_byte(0)
  end

  @doc """
  Calculates the remaining length (as specified in MQTT spec) for a given Message struct
  """
  def calculate_remaining_length(%Message.Connect{
        client_id: client_id,
        username: username,
        password: password,
    ...
    variable_header_length + client_id_length + will_topic_length + will_message_length +
  ...
  def calculate_remaining_length(%Message.Subscribe{topics: topics}) do
    ...
    topic_length_byte_length = 2
    qos_length = 1
    topic_qos_length = fn t -> byte_size(t) + topic_length_byte_length + qos_length end
  ...
  """
  def encode_variable_header(%Message.Connect{
        username: username,
        password: password,
        will_qos: will_qos,
  ...
  def encode_variable_header(%Message.ConnAck{
  ...
  """
  def encode_payload(%Message.Connect{
        client_id: id,
        username: username,
        password: password,
        will_message: will_message,
    ...
    client_id_load <> will_topic_load <> will_msg_load <> username_load <> password_load
  end
